...
```{r}
sapply(1:reps, function(i){sum(rexp(n=nexps, rate=rate))})
```

reps= hversu margar endurtekningar
...
```{r}
teikna_medaltol <- function (x) 
  {
  x <- mean(rnorm(100))
  xdata <- as.data.frame(x)
  plot <- ggplot(xdata, aes(x=x)) + geom_histogram() + theme_classic()
  return(plot)
  }

```

ég hef ekki hugmynd hvað ég er að gera...
